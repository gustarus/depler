...
function extractBlocks(template, expressionSome, expressionOpen, expressionClose) {
...
function processBlocks(template, blocks, callback) {
    // process loops
...
function processTemplate(template, variables, context) {
    let result = template;
    ...
    const propertiesObject = properties.reduce((stack, name) => (stack[name] = true) && stack, {});
    ...
    const globalsObject = globals.reduce((stack, name) => (stack[name] = true) && stack, {});
